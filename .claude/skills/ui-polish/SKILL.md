---
name: ui-polish
description: 收斂畫面的間距 / 尺寸 / 圓角 / 字級 / 行高 / 對齊到本專案 appTheme 設計 token（手機舒適度優先、不忽大忽小）。TRIGGER 當使用者說「排版怪怪的」「間距調一下」「比例不協調」「字太大 / 太小」「這頁 polish 一下」「元件不一致」時使用。不改文案、顏色語意、結構與邏輯。
---
...
2. **生 subagent 做 Inventory + 提案**：用 Agent 工具、`subagent_type: "ui-polish"`，請它列出目標檔的間距 / 圓角 / 字級離群值，並產出 `before → after(token) → 依據` 清單（**先不要動手**）。多頁就平行生多個。
...
- 手機舒適度：正文 / 商品名不低於 14，次要資訊 12，微型 chrome 9–11（例外要 flag）；同類元素固定同一級、不忽大忽小。
